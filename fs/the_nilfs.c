 ...
 *
 * the_nilfs.c,v 1.72 2008-02-06 07:20:30 ryusuke Exp
 *
...
int init_nilfs(struct the_nilfs *nilfs, struct nilfs_sb_info *sbi, char *data)
{
	...
	nilfs_debug(2, "checking magic number\n");
	err = nilfs_store_magic_and_option(sb, sbp, data);
	if (err)
		goto failed_sbh;
...
void nilfs_dispose_used_segments(struct the_nilfs *nilfs)
{
	...
	if (!nilfs_loaded(nilfs)) {
		nilfs_debug(1, "the_nilfs is not loaded (ignored).\n");
		return;
	}
	list_for_each_entry_safe(ent, n, &nilfs->ns_used_segments, list) {
		list_del_init(&ent->list);
		nilfs_debug(3, "remove volatile active segment (seg#=%llu)\n",
