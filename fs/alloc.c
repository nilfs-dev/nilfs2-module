...
{
	return sector_div(group, nilfs_persistent_group_descs_per_block(inode));
}
			 ...
			 const struct buffer_head *desc_bh)
{
	...
	return (struct nilfs_persistent_group_desc *)(kaddr + bh_offset(desc_bh)) +
		nilfs_persistent_desc_offset(inode, group);
}

static void
nilfs_persistent_desc_block_init(struct inode *inode, struct buffer_head *bh, void *kaddr)
{
	struct nilfs_persistent_group_desc *desc = kaddr + bh_offset(bh);
	int i;

	for (i = 0; i < nilfs_persistent_group_descs_per_block(inode); i++) {
		desc->pg_nfrees = cpu_to_le32(nilfs_persistent_entries_per_group(inode));
		desc++;
	}
}
...
{
	nilfs_blkoff_t blkoff = nilfs_persistent_group_desc_blkoff(inode, group);

	return nilfs_mdt_get_block(inode, blkoff, 1, nilfs_persistent_desc_block_init,
				   desc_bhp);
}

static int
nilfs_persistent_get_group_bitmap_block(struct inode *inode, nilfs_bgno_t group,
					struct buffer_head **bitmap_bhp)
{
	nilfs_blkoff_t blkoff = nilfs_persistent_group_bitmap_blkoff(inode, group);
				     ...
				     int bsize)
{
	...
	if (target > 0) {
		...
		if ((pos < end) &&
		    !nilfs_persistent_set_bit_atomic(nilfs_mdt_bgl_lock(inode, group), pos, bitmap))
			return pos;
	} else
		end = 0;
	...
	for (i = 0, curr = end;
		...
		while (*((unsigned long *)bitmap + curr / BITS_PER_LONG) != ~0UL) {
			...
			pos = nilfs_persistent_find_next_zero_bit(bitmap, end, curr);
			if ((pos < end) &&
			    !nilfs_persistent_set_bit_atomic(nilfs_mdt_bgl_lock(inode, group), pos, bitmap))
				return pos;
		}
...
int
nilfs_persistent_prepare_alloc_entry(struct inode *inode, struct nilfs_persistent_req *req,
				     nilfs_bgno_t *group_p, int *target_p)
{
	...
	for (i = 0; i < ngroups; i++, group++) {
		if (group >= ngroups)
			group = 0;
		if ((ret = nilfs_persistent_get_group_desc_block(inode, group, &desc_bh)) < 0)
			return ret;
		desc = nilfs_persistent_get_group_desc(inode, group, desc_bh);
		if (le32_to_cpu(desc->pg_nfrees) > 0) {
			if ((ret = nilfs_persistent_get_group_bitmap_block(inode, group, &bitmap_bh)) < 0) {
				nilfs_persistent_put_group_desc(inode, desc_bh);
				nilfs_persistent_put_group_desc_block(inode, desc_bh);
				return ret;
			}
			start = nilfs_persistent_get_group_bitmap_buffer(inode, bitmap_bh);
			if ((pos = nilfs_persistent_group_find_available_slot(
				     		inode, group, target, start, bsize)) >= 0) {
			...
			nilfs_persistent_put_group_bitmap_buffer(inode, bitmap_bh);
			nilfs_persistent_put_group_bitmap_block(inode, bitmap_bh);
		}
		nilfs_persistent_put_group_desc(inode, desc_bh);
		nilfs_persistent_put_group_desc_block(inode, desc_bh);
		target = 0;
...
void
nilfs_persistent_commit_alloc_entry(struct inode *inode, struct nilfs_persistent_req *req)
{
...
void
nilfs_persistent_abort_alloc_entry(struct inode *inode, struct nilfs_persistent_req *req,
				   nilfs_bgno_t group, int grpoff)
{
	...
	bitmap_buffer = nilfs_persistent_get_group_bitmap_buffer(inode, req->pr_bitmap_bh);

	if (!nilfs_persistent_clear_bit_atomic(nilfs_mdt_bgl_lock(inode, group),
...
int
nilfs_persistent_prepare_free_entry(struct inode *inode, struct nilfs_persistent_req *req, nilfs_bgno_t group)
{
	struct buffer_head *desc_bh, *bitmap_bh;
	int ret;
...
void
nilfs_persistent_abort_free_entry(struct inode *inode, struct nilfs_persistent_req *req)
{
	nilfs_persistent_put_group_bitmap_buffer(inode, req->pr_bitmap_bh);
	nilfs_persistent_put_group_bitmap_block(inode, req->pr_bitmap_bh);
	nilfs_persistent_put_group_desc(inode, req->pr_desc_bh);
